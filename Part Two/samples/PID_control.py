...
class Vehicle2D(object):
    ...
    def move(self, steering, distance, tolerance=0.001, max_steering_angle=np.pi / 4.0):
        ...
        if max_steering_angle > np.pi/2:
            raise ValueError(
                "Max steering angle cannot be greater than 90-degree.")
    ...
    def compute_error(self, track):
        ...
        for i in range(len(track)):
            distance = np.linalg.norm(
                np.array([self.x, self.y]) - np.array(track[i]))
...
K_d = 3
K_i = 0.0005
x_trajectory, y_trajectory = vehicle.run_PID(
    targets, K_p, K_d, K_i, track_length)
n = len(x_trajectory)
